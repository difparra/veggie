...
import com.diegoparra.veggie.auth.usecases.auth.EmailAuthUseCase
...
abstract class EmailAuthViewModel<Params : EmailAuthUseCase.EmailParams>(
    ...
    protected open fun handleFailure(failure: Failure) {
        ...
        when (failure) {
            is AuthFailure -> handleAuthFailure(failure)
            else -> _toastFailure.value = Event(failure)
        }
    }

    private fun handleAuthFailure(failure: AuthFailure) {
        when (failure) {
            is AuthFailure.ValidationFailures -> failure.failures.forEach { handleFailure(it) }
            is AuthFailure.WrongInput -> handleInputFailure(failure.field, failure)
            is AuthFailure.WrongSignInMethod -> _email.value = Resource.Error(failure)
            is AuthFailure.SignInState -> _toastFailure.value = Event(failure)
            is AuthFailure.PhoneAuthFailures -> Timber.e("There shouldn't be phone Failures in here, check.")
            is AuthFailure.ServerError -> _toastFailure.value = Event(failure)
        }
    }

    private fun handleInputFailure(field: String, failure: AuthFailure.WrongInput) {
    ...
    protected abstract fun handleInputFailureOnAdditionalFields(field: String, failure: AuthFailure.WrongInput)
