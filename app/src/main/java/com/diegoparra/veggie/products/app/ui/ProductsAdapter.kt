...
import com.diegoparra.veggie.products.app.ui.utils.*
...
class ProductsAdapter : ListAdapter<ProductMain, ProductsAdapter.ProductViewHolder>(DiffCallback) {
    ...
    inner class ProductViewHolder(private var binding: ListItemMainProductBinding) :
        RecyclerView.ViewHolder(binding.root) {
        init {
            binding.btnQty.setOnClickListener { view ->
                val position = bindingAdapterPosition
                val item =
                    if (position != RecyclerView.NO_POSITION) getItem(bindingAdapterPosition) else null
                item?.let { product ->
        ...
        private fun navigateToProductDetails(product: ProductMain, view: View) {
            ...
            val action = NavMainDirections.actionGlobalToProductDetailsFragment(
