...
class GoogleAuthUseCase @Inject constructor(
    ...
    override suspend fun validate(params: Either<ApiException, GoogleSignInAccount>): Either<AuthFailure.ValidationFailures, Unit> {
        //  There are no previous validations to perform when signing in with Google
        return Either.Right(Unit)
    }

    override suspend fun authenticate(params: Either<ApiException, GoogleSignInAccount>): Either<AuthFailure, AuthResults> {
        return when (params) {
            is Either.Left -> Either.Left(AuthFailure.ServerError(params.a))
